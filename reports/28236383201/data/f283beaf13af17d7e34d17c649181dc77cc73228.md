...
- Name: practice-software-testing/checkout.spec.js >> Checkout Flow Module >> [TC-PST-CHK-003] Verifikasi pemotongan stok pasca-checkout
- Location: tests/practice-software-testing/checkout.spec.js:65:5
...
Call log:
  - waiting for locator('[data-test="add-to-cart"]') to be visible
...
```ts
  ...
  16 |         await page.waitForSelector('.card', { state: 'visible' });
  17 |         
  18 |         // Add a product to cart as a prerequisite
  19 |         await page.locator('.card').first().click();
> 20 |         await page.waitForSelector('[data-test="add-to-cart"]');
  ...
  71 |         await page.locator('.card').first().click();
