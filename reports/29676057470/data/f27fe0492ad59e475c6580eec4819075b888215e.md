...
- Name: practice-software-testing/cart.spec.js >> Shopping Cart Module >> [TC-PST-CRT-002] Add to Cart melebihi stok yang ada
- Location: tests/practice-software-testing/cart.spec.js:28:5
...
Call log:
  - waiting for locator('[data-test="increase-quantity"]') to be visible
      ...
      - generic [ref=e183]:
        - generic [ref=e184]:
          - 'link "Combination Pliers Compare Combination Pliers CO₂: A B C D E $14.15" [ref=e185] [cursor=pointer]':
            - /url: /product/01KXWFC4G2CHA35SBTSB3C13N4
            - generic [ref=e186]:
          ...
          - 'link "Pliers Compare Pliers CO₂: A B C D E $12.01" [ref=e202] [cursor=pointer]':
            - /url: /product/01KXWFC4G6R11VHQAHYMAB8WCY
            - generic [ref=e203]:
              - img "Pliers" [ref=e204]
          ...
          - 'link "Bolt Cutters Compare Bolt Cutters CO₂: A B C D E $48.41" [ref=e219] [cursor=pointer]':
            - /url: /product/01KXWFC4G8XMEV91FPK8N8MHXG
            - generic [ref=e220]:
          ...
          - 'link "Long Nose Pliers Compare Long Nose Pliers CO₂: A B C D E Out of stock $14.24" [ref=e236] [cursor=pointer]':
            - /url: /product/01KXWFC4G9YJJW7HS4Q3D47SQA
            - generic [ref=e237]:
          ...
          - 'link "Slip Joint Pliers Compare Slip Joint Pliers CO₂: A B C D E $9.17" [ref=e254] [cursor=pointer]':
            - /url: /product/01KXWFC4GBGACMAF78FP5QR72E
            - generic [ref=e255]:
          ...
          - 'link "Claw Hammer with Shock Reduction Grip Compare Claw Hammer with Shock Reduction Grip CO₂: A B C D E $13.41" [ref=e271] [cursor=pointer]':
            - /url: /product/01KXWFC4GC0X6J152MNG52G943
            - generic [ref=e272]:
          ...
          - 'link "Hammer Compare Hammer CO₂: A B C D E $12.58" [ref=e288] [cursor=pointer]':
            - /url: /product/01KXWFC4GEC3EZ63ZY5J85AEK3
            - generic [ref=e289]:
              - img "Hammer" [ref=e290]
          ...
          - 'link "Claw Hammer Compare Claw Hammer CO₂: A B C D E $11.48" [ref=e305] [cursor=pointer]':
            - /url: /product/01KXWFC4GFXYZKGC5NHF7JE4ZS
            - generic [ref=e306]:
          ...
          - 'link "Thor Hammer Compare Thor Hammer CO₂: A B C D E $11.14" [ref=e322] [cursor=pointer]':
            - /url: /product/01KXWFC4GHC1XQAYHZEWQ8NCV5
            - generic [ref=e323]:
...
```ts
  ...
  2  | 
  3  | test.describe('Shopping Cart Module', () => {
  4  |     let baseURL;
  5  | 
  6  |     test.beforeAll(() => {
  ...
  12 |         await page.waitForSelector('.card', { state: 'visible' });
  13 |     });
  14 | 
  15 |     test('[TC-PST-CRT-001] Add to Cart dari list produk', async ({ page }) => {
  16 |         // Find the first product card and click it to go to details
  17 |         await page.locator('.card').first().click();
  18 |         
  19 |         // Wait for detail page
  20 |         await page.waitForSelector('[data-test="add-to-cart"]');
  21 |         await page.locator('[data-test="add-to-cart"]').click();
  22 | 
  23 |         // Verify toast or cart counter
  24 |         const cartBadge = page.locator('[data-test="cart-quantity"]');
  25 |         await expect(cartBadge).toHaveText(/1|[1-9]/); // greater than 0
  26 |     });
  27 | 
  28 |     test('[TC-PST-CRT-002] Add to Cart melebihi stok yang ada', async ({ page }) => {
  29 |         await page.locator('.card').first().click();
  30 |         
> 31 |         await page.waitForSelector('[data-test="increase-quantity"]');
     |                    ^ Error: page.waitForSelector: Test timeout of 30000ms exceeded.
  32 |         
  33 |         // Let's assume stock is limited, we try to click increase 50 times
  34 |         for (let i = 0; i < 50; i++) {
  35 |             await page.locator('[data-test="increase-quantity"]').click();
  36 |         }
  37 | 
  38 |         await page.locator('[data-test="add-to-cart"]').click();
  39 | 
  40 |         // Should either show error toast or not allow to go past stock
  41 |         const errorToast = page.locator('.toast-error, [data-test="toast-error"]');
  42 |         if (await errorToast.count() > 0) {
  43 |             await expect(errorToast).toBeVisible();
  44 |         }
  45 |     });
  46 | 
  47 |     test('[TC-PST-CRT-003] Update Qty di halaman Cart', async ({ page }) => {
  48 |         // Add a product first
  49 |         await page.locator('.card').first().click();
  50 |         await page.waitForSelector('[data-test="add-to-cart"]');
  51 |         await page.locator('[data-test="add-to-cart"]').click();
  52 |         
  53 |         // Go to cart
  54 |         await page.goto(baseURL + '#/checkout');
  55 |         await page.waitForSelector('[data-test="cart-item"]');
  56 | 
  57 |         // Increase qty inside cart (depends on UI, assuming an input field or button)
  58 |         const qtyInput = page.locator('input[type="number"], [data-test="item-quantity"]').first();
  59 |         if (await qtyInput.count() > 0) {
  60 |             await qtyInput.fill('3');
  61 |             // Check if subtotal is updated
  62 |             await page.waitForLoadState('networkidle');
  63 |             await expect(page.locator('[data-test="cart-total"]')).toBeVisible();
  64 |         }
  65 |     });
  66 | });
  67 | 
