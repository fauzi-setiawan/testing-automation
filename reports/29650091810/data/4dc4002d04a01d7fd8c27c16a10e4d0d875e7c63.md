...
- Name: practice-software-testing/auth.spec.js >> Authentication Module >> [TC-PST-AUTH-004] Register akun baru valid
- Location: tests/practice-software-testing/auth.spec.js:48:5
...
Call log:
  - waiting for locator('[data-test="first-name"]')
      ...
      - generic [ref=e183]:
        - generic [ref=e184]:
          - 'link "Combination Pliers Compare Combination Pliers CO₂: A B C D E $14.15" [ref=e185] [cursor=pointer]':
            - /url: /product/01KXTVW5KY6G5QBZ7P2ERXXB5C
            - generic [ref=e186]:
          ...
          - 'link "Pliers Compare Pliers CO₂: A B C D E $12.01" [ref=e202] [cursor=pointer]':
            - /url: /product/01KXTVW5M341V4Z2S2WJKVMM6G
            - generic [ref=e203]:
              - img "Pliers" [ref=e204]
          ...
          - 'link "Bolt Cutters Compare Bolt Cutters CO₂: A B C D E $48.41" [ref=e219] [cursor=pointer]':
            - /url: /product/01KXTVW5M51YS09KM2CMNXRT04
            - generic [ref=e220]:
          ...
          - 'link "Long Nose Pliers Compare Long Nose Pliers CO₂: A B C D E Out of stock $14.24" [ref=e236] [cursor=pointer]':
            - /url: /product/01KXTVW5M752H0T1TGXJMENPX5
            - generic [ref=e237]:
          ...
          - 'link "Slip Joint Pliers Compare Slip Joint Pliers CO₂: A B C D E $9.17" [ref=e254] [cursor=pointer]':
            - /url: /product/01KXTVW5M9W72AD60GWERA797A
            - generic [ref=e255]:
          ...
          - 'link "Claw Hammer with Shock Reduction Grip Compare Claw Hammer with Shock Reduction Grip CO₂: A B C D E $13.41" [ref=e271] [cursor=pointer]':
            - /url: /product/01KXTVW5MB20K697D2X3XXC9JW
            - generic [ref=e272]:
          ...
          - 'link "Hammer Compare Hammer CO₂: A B C D E $12.58" [ref=e288] [cursor=pointer]':
            - /url: /product/01KXTVW5MDWN6MYZEM49MQW7P8
            - generic [ref=e289]:
              - img "Hammer" [ref=e290]
          ...
          - 'link "Claw Hammer Compare Claw Hammer CO₂: A B C D E $11.48" [ref=e305] [cursor=pointer]':
            - /url: /product/01KXTVW5ME9PQPW61ZDMQ8VDXG
            - generic [ref=e306]:
          ...
          - 'link "Thor Hammer Compare Thor Hammer CO₂: A B C D E $11.14" [ref=e322] [cursor=pointer]':
            - /url: /product/01KXTVW5MG7TH5WFB5FVYZQ6J8
            - generic [ref=e323]:
...
```ts
  ...
  2  | 
  3  | test.describe('Authentication Module', () => {
  4  |     let baseURL;
  5  |     let testEmail;
  6  |     let testPassword;
  7  | 
  ...
  15 |         // Go to login page
  16 |         await page.goto(baseURL + '#/auth/login');
  17 |     });
  18 | 
  19 |     test('[TC-PST-AUTH-001] Login dengan kredensial valid', async ({ page }) => {
  20 |         await page.locator('[data-test="email"]').fill(testEmail);
  21 |         await page.locator('[data-test="password"]').fill(testPassword);
  22 |         await page.locator('[data-test="login-submit"]').click();
  23 |         
  24 |         // Assert redirect to My Account or Account details
  25 |         await expect(page).toHaveURL(/.*\/account/);
  26 |         await expect(page.locator('[data-test="nav-menu"]')).toContainText('fauzisetiawan75');
  27 |     });
  28 | 
  29 |     test('[TC-PST-AUTH-002] Login dengan password yang salah', async ({ page }) => {
  30 |         await page.locator('[data-test="email"]').fill(testEmail);
  31 |         await page.locator('[data-test="password"]').fill('WrongPassword123!');
  32 |         await page.locator('[data-test="login-submit"]').click();
  33 | 
  34 |         // Assert error message
  35 |         const errorAlert = page.locator('[data-test="login-error"]');
  36 |         await expect(errorAlert).toBeVisible();
  37 |         await expect(errorAlert).toContainText('Invalid email or password');
  38 |     });
  39 | 
  40 |     test('[TC-PST-AUTH-003] Login dengan field kosong', async ({ page }) => {
  41 |         await page.locator('[data-test="login-submit"]').click();
  42 | 
  43 |         // Assert validation error
  44 |         await expect(page.locator('[data-test="email-error"]')).toBeVisible();
  45 |         await expect(page.locator('[data-test="password-error"]')).toBeVisible();
  46 |     });
  47 | 
  48 |     test('[TC-PST-AUTH-004] Register akun baru valid', async ({ page }) => {
  49 |         await page.goto(baseURL + '#/auth/register');
  50 | 
  51 |         const uniqueEmail = `testuser${Date.now()}@test.com`;
  52 | 
> 53 |         await page.locator('[data-test="first-name"]').fill('Test');
     |                                                        ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  54 |         await page.locator('[data-test="last-name"]').fill('User');
  55 |         await page.locator('[data-test="dob"]').fill('1990-01-01');
  56 |         await page.locator('[data-test="address"]').fill('123 Test Street');
  57 |         await page.locator('[data-test="postcode"]').fill('12345');
  58 |         await page.locator('[data-test="city"]').fill('Test City');
  59 |         await page.locator('[data-test="state"]').fill('Test State');
  60 |         await page.locator('[data-test="country"]').selectOption('US');
  61 |         await page.locator('[data-test="phone"]').fill('1234567890');
  62 |         await page.locator('[data-test="email"]').fill(uniqueEmail);
  63 |         await page.locator('[data-test="password"]').fill('ValidPass123!');
  64 |         await page.locator('[data-test="register-submit"]').click();
  65 | 
  66 |         // Assert registration success, usually redirected to login
  67 |         await expect(page).toHaveURL(/.*\/auth\/login/);
  68 |     });
  69 | });
  70 | 
```